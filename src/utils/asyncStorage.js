...
import {ACCESS_TOKEN, ON_BOARDING, LETS_GET_STARTED} from './keys';
...
const StorageValue = async () => {
  ...
  const letsGetStarted = !!asyncData[2][1] ? asyncData[2][1] : false;
  return {OnBoardingDataValue, authDataValue, letsGetStarted};
};
...
const letsGetStarted = async value => {
  const stringData = JSON.stringify(value);
  await AsyncStorage.setItem(LETS_GET_STARTED, stringData);
  return;
};

export {OnBoardingToken, authToken, letsGetStarted, StorageValue};
